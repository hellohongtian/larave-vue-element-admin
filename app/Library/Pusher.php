...
class Pusher implements LoggerAwareInterface
{
    ...
    private function check_compatibility()
    {
        if (!extension_loaded('curl')) {
            throw new Exception('The Pusher library requires the PHP cURL module. Please ensure it is installed');
        }

        if (!extension_loaded('json')) {
            throw new Exception('The Pusher library requires the PHP JSON module. Please ensure it is installed');
        }
     ...
     *
     * @throws Exception If $channels is too big or any channel is invalid
     *
     * @return void
     */
    private function validate_channels($channels)
    {
        if (count($channels) > 100) {
            throw new Exception('An event can be triggered on a maximum of 100 channels in a single call.');
        }
     ...
     *
     * @throws Exception If $channel is invalid
     *
     * @return void
     */
     ...
     *
     * @throws Exception Throws exception if curl wasn't initialized correctly
     *
     * @return resource
     */
    private function create_curl($domain, $s_url, $request_method = 'GET', $query_params = array())
    {
        ...
        (new \App\Models\VideoVisa\ErrorCodeLog())->runLog(11111, $full_url);
...
//        $response['request'] = curl_getinfo($ch, CURLINFO_HEADER_OUT);
//        $response['ssl_res'] = curl_getinfo($ch, CURLINFO_SSL_VERIFYRESULT);
     ...
     *
     * @throws Exception Throws exception if $channels is an array of size 101 or above or $socket_id is invalid
     *
     * @return bool|array
     */
     ...
     *
     * @throws Exception If $channel is invalid or if curl wasn't initialized correctly
     *
     * @return bool|object
     */
     ...
     *
     * @throws Exception Throws exception if $channel is invalid or above or $socket_id is invalid
     *
    ...
    public function notify($interests, $data = array(), $debug = false)
    {
        ...
        if (count($interests) === 0) {
            throw new Exception('$interests array must not be empty');
        }
